...
def test_5_1_training_pipeline():
    ...
    try:
        ...
        print(f"   CARLA config: {carla_config['map']}")
        print(f"   TD3 config: {td3_config.get('algorithm', 'TD3')}")
        ...
        agent = TD3Agent(
            ...
            max_action=1.0,
            device=device,
            learning_rate=td3_config.get('learning_rate', 3e-4),
            tau=td3_config.get('tau', 0.005),
            policy_freq=td3_config.get('policy_freq', 2)
        )
        print(f"✅ Agent ready (device={device})")
        print(f"   Learning rate: {td3_config.get('learning_rate', 3e-4)}")
        print(f"   Target update rate (tau): {td3_config.get('tau', 0.005)}")
        print(f"   Policy update freq: {td3_config.get('policy_freq', 2)}")
        
        # Training parameters
        TOTAL_STEPS = 100
        RANDOM_EXPLORE_STEPS = 25
        TRAINING_START_STEP = 50
        BATCH_SIZE = 64
