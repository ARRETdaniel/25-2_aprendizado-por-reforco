...
docker start carla-server && xhost +local:docker 2>/dev/null || echo "xhost not available, proceeding anyway"
...
## python API
 cd /media/danielterra/Windows-SSD/Users/danie/Documents/Documents/MESTRADO/25-2_aprendizado-por-reforco/Detalhamento/av_td3_system && docker run --rm --network host --runtime nvidia -e NVIDIA_VISIBLE_DEVICES=all -e NVIDIA_DRIVER_CAPABILITIES=all -e PYTHONUNBUFFERED=1 -e PYTHONPATH=/workspace -e DISPLAY=$DISPLAY -e QT_X11_NO_MITSHM=1 -v /tmp/.X11-unix:/tmp/.X11-unix:rw -v $(pwd):/workspace -w /workspace --privileged av-td3-system:ubuntu22.04-test bash -c "source /opt/ros/humble/setup.bash && python3 scripts/evaluate_baseline.py --scenario 0 --num-episodes 3 --baseline-config config/baseline_config.yaml --debug" 2>&1 | tee docs/day-26/paper/roboticaMovel-paper.log
...
cd /media/danielterra/Windows-SSD/Users/danie/Documents/Documents/MESTRADO/25-2_aprendizado-por-reforco/Detalhamento/av_td3_system && docker run --rm --gpus all --network=host \
  ...
  -w /workspace \
  --privileged \
  av-td3-system:ubuntu22.04-test python3 scripts/evaluate_baseline.py --use-ros-bridge --num-episodes 3 --debug 2>&1  2>&1 | tee /media/danielterra/Windows-SSD/Users/danie/Documents/Documents/MESTRADO/25-2_aprendizado-por-reforco/Detalhamento/av_td3_system/docs/day-27/baseline-ROS2.log
