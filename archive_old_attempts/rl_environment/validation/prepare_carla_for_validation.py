...
Usage:
    ...
    python prepare_carla_for_validation.py --check-only
    python prepare_carla_for_validation.py --map /Game/Maps/Town02 --fps 60
...
def find_carla_executable() -> Optional[str]:
    ...
    return Nonedef is_port_in_use(port: int) -> bool:
    """
...
def run_validation_tests(
    ...
    """
    try:
        # Build the command to run validation tests
        cmd = [
            sys.executable,  # Current Python interpreter
            "-m",
            "rl_environment.validation.run_validation",
            f"--host={host}",
            f"--port={port}",
